...
import {
  Activity, Check, ChevronDown, Download, FileImage, Gauge, Globe2, Import,
  LoaderCircle, Play, Plus, RefreshCw, Search, Server, Settings2, Trash2, X
} from 'lucide-react';
    ...
    <table className="w-full min-w-[1180px] border-collapse text-sm">
      ...
      <tbody>
        ...
        {nodes.map(status => <tr key={status.node.id} className="border-t hover:bg-muted/35">
          ...
          <td className="px-3 py-2.5"><span className={`inline-flex items-center gap-1.5 ${status.online ? 'text-emerald-700' : 'text-red-700'}`}><span className={`size-1.5 rounded-full ${status.online ? 'bg-emerald-500' : 'bg-red-500'}`} />{status.online ? '在线' : '离线'}</span></td>
...
function SettingsView({ config, onSaved, notify }) {
  const [form, setForm] = useState(config || { checkIntervalMinutes: 5, timeoutSeconds: 10, concurrency: 8 });
  useEffect(() => setForm(config || form), [config]);
  const save = async event => { event.preventDefault(); try { await api('/api/system/monitoring', { method: 'PUT', body: JSON.stringify(form) }); notify('设置已保存'); await onSaved(); } catch (error) { notify(error.message, true); } };
  return <form onSubmit={save} className="max-w-2xl">
    <div className="mb-5"><h2 className="text-base font-semibold">检测设置</h2><p className="mt-1 text-sm text-muted-foreground">调整后下一个周期生效。</p></div>
    <div className="grid gap-5 sm:grid-cols-3">
      <label className="grid gap-2 text-sm font-medium">周期（分钟）<Input type="number" min="1" value={form.checkIntervalMinutes ?? 5} onChange={e => setForm({ ...form, checkIntervalMinutes: Number(e.target.value) })} /></label>
      <label className="grid gap-2 text-sm font-medium">超时（秒）<Input type="number" min="1" value={form.timeoutSeconds ?? 10} onChange={e => setForm({ ...form, timeoutSeconds: Number(e.target.value) })} /></label>
      <label className="grid gap-2 text-sm font-medium">并发数<Input type="number" min="1" max="32" value={form.concurrency ?? 8} onChange={e => setForm({ ...form, concurrency: Number(e.target.value) })} /></label>
    </div>
    <Button className="mt-6" type="submit"><Check />保存设置</Button>
  </form>;
}
...
function App() {
  ...
  const load = async () => {
    const [nodeData, subData, importData, systemData, configData] = await Promise.all([
      api('/api/nodes/public'), api('/api/subscriptions'), api('/api/imports'), api('/api/system/status'), api('/api/system/config')
    ]);
    setNodes(nodeData.stats); setSummary(nodeData.summary); setSubscriptions(subData.subscriptions);
    setImports(importData.imports); setSystem(systemData.status); setMonitoring(configData.config.monitoring);
  };
  ...
  const filtered = useMemo(() => nodes.filter(item => {
    ...
    return (!query || `${item.node.name} ${item.node.server} ${item.node.protocol}`.toLowerCase().includes(query))
      ...
      && (statusFilter === 'all' || (statusFilter === 'online') === item.online);
  ...
  return <TooltipProvider><div className="min-h-screen">
    ...
    <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6">
      ...
      <section className="mb-6 flex divide-x border-b">
        ...
        <Metric label="离线" value={summary.offline} detail="最近一轮检测" tone={summary.offline ? 'text-red-700' : ''} />
      ...
      <Tabs defaultValue="nodes">
        ...
        <TabsContent value="settings"><SettingsView config={monitoring} onSaved={load} notify={notify} /></TabsContent>
      </Tabs>
    </main>
  </div></TooltipProvider>;
}
